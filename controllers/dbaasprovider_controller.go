...
type DBaaSProviderReconciler struct {
	...
	*runtime.Scheme
	...
	dbaasCdrChecker          func(gv, k string) (bool, error)
}

const (
	resourceName      = "cockroachdb-cloud-registration"
...
func (r *DBaaSProviderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := log.FromContext(ctx, "CockroachDB DBaaSProvider Reconciler", req.NamespacedName)
	...
	if r.dbaasCdrChecker == nil {
		r.dbaasCdrChecker = r.checkCrdInstalled
	}
	isCrdInstalled, err := r.dbaasCdrChecker(dbaasoperator.GroupVersion.String(), dbaasProviderKind)
	if err != nil {
		log.Error(err, "error while discovering DBaaS GVK")
		return ctrl.Result{}, err
	}
	if !isCrdInstalled {
		log.Info("CRD not found, requeuing with rate limiter")
...
// TODO can this put in a configmap?
	...
	instance := &dbaasoperator.DBaaSProvider{
		...
		Spec: dbaasoperator.DBaaSProviderSpec{
			Provider: dbaasoperator.DatabaseProvider{
				...
				DisplayDescription: "Some description for CockroachDb provider",
